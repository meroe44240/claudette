...
export default async function candidatureRouter(fastify: FastifyInstance) {
  ...
  fastify.get('/', {
    ...
    handler: async (request) => {
      ...
      const stage = query.stage ? z.enum(['SOURCING', 'CONTACTE', 'ENTRETIEN_1', 'ENVOYE_CLIENT', 'ENTRETIEN_CLIENT', 'PROCESS', 'OFFRE', 'PLACE', 'REFUSE']).parse(query.stage) : undefined;
  ...
  fastify.post('/bulk-stage', {
    ...
    handler: async (request) => {
      const input = z.object({
        ...
        stage: z.enum(['SOURCING', 'CONTACTE', 'ENTRETIEN_1', 'ENVOYE_CLIENT', 'ENTRETIEN_CLIENT', 'PROCESS', 'OFFRE', 'PLACE', 'REFUSE']),
